"""Run a single A-fix cell by index. Mirrors `_run_offload_cell.py`."""
...
if __name__ == "__main__":
    ...
    from _a_fix_cells import all_cells
    cells = all_cells()
    if cell_idx >= len(cells):
        print(f"[run-a-fix] cell_idx {cell_idx} out of range (have {len(cells)})", file=sys.stderr)
        sys.exit(2)
    cell = cells[cell_idx]
    args = cell["args"]
    ...
    if result_path.exists() and result_path.stat().st_size > 0:
        print(f"[run-a-fix] cell {cell_idx}: SKIP (already complete)")
        sys.exit(0)
    env = os.environ.copy()
