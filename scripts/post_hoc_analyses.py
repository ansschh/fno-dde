...
def aggregate_and_plot(layer_root: Path):
    ...
    eps_lists = []
    rel_lists = []
    for f in Path(layer_root).rglob("adv_fgsm.json"):
        d = json.loads(f.read_text())
        eps_lists.append(d.get("epsilons", []))
        rel_lists.append(d.get("rel_l2_per_eps", []))
    if eps_lists:
        eps = eps_lists[0]
        L = min(len(r) for r in rel_lists)
        arr = np.array([r[:L] for r in rel_lists])
        fig, ax = plt.subplots(figsize=(6, 3.5))
        ax.plot(eps[:L], arr.mean(axis=0), "o-", color="#d62728", lw=1.5)
        ax.fill_between(eps[:L], arr.mean(axis=0) - arr.std(axis=0),
                         arr.mean(axis=0) + arr.std(axis=0),
                         color="#d62728", alpha=0.25)
        ax.set_xlabel(r"FGSM perturbation $\varepsilon$")
        ax.set_ylabel(r"test rel$L_2$")
        ax.set_yscale("log")
        ax.set_title(r"Adversarial FGSM robustness (LEMO-PC, mean $\pm$ std over cells)")
        ax.grid(linestyle="--", alpha=0.4)
        fig.tight_layout()
        out = fig_dir / "F11_adversarial.pdf"
        fig.savefig(out)
        fig.savefig(out.with_suffix(".png"), dpi=150)
        plt.close(fig)
        print(f"  -> {out.name}")
    ...
    nl_lists = []
    rel_lists = []
    for f in Path(layer_root).rglob("noise_sweep.json"):
        d = json.loads(f.read_text())
        nl_lists.append(d.get("noise_levels", []))
        rel_lists.append(d.get("rel_l2_per_noise", []))
    if nl_lists:
        nl = nl_lists[0]
        L = min(len(r) for r in rel_lists)
        arr = np.array([r[:L] for r in rel_lists])
        fig, ax = plt.subplots(figsize=(6, 3.5))
        ax.plot(nl[:L], arr.mean(axis=0), "o-", color="#1f77b4", lw=1.5)
        ax.fill_between(nl[:L], arr.mean(axis=0) - arr.std(axis=0),
                         arr.mean(axis=0) + arr.std(axis=0),
                         color="#1f77b4", alpha=0.25)
        ax.set_xlabel(r"input noise $\sigma$")
        ax.set_ylabel(r"test rel$L_2$")
        ax.set_yscale("log")
        ax.set_title(r"Noise robustness (LEMO-PC, mean $\pm$ std over cells)")
        ax.grid(linestyle="--", alpha=0.4)
        fig.tight_layout()
        out = fig_dir / "F12_noise_sweep.pdf"
        fig.savefig(out)
        fig.savefig(out.with_suffix(".png"), dpi=150)
        plt.close(fig)
        print(f"  -> {out.name}")
