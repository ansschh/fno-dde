...
def main():
    ...
    for model in MODEL_COLOR:
        ...
        ts_m = np.arange(len(m))
        ax.plot(ts_m, m, color=color, lw=1.7,
                label=f"{MODEL_LABEL[model]} (b/i = {d['ratio']:.2f})")
        ax.fill_between(ts_m, np.maximum(m - s, 1e-5), m + s,
                        color=color, alpha=0.13, linewidth=0)
        plotted.append(model)
    # Boundary shading after data plotted (so axes are set)
    ax.axvspan(0, B - 0.5, color="grey", alpha=0.12, lw=0,
               label=f"boundary (t<{B} or t≥T-{B})")
    ax.axvspan(T - B - 0.5, T - 1, color="grey", alpha=0.12, lw=0)
